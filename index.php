...
function make_guest_order($product, $pid, $quantity, $customer)
{
...
if (isset($_POST['action'])) {
  ...
  if ($action === 'order') {
    ...
    if (!isset($catalog[$pid])) {
      echo json_encode(['ok' => false, 'msg' => 'محصول انتخاب‌شده معتبر نیست.']);
      exit;
    }
    ...
    if (strlen($customer['phone']) < 10) {
      echo json_encode(['ok' => false, 'msg' => 'شماره تماس معتبر نیست.']);
      exit;
    }
...
<head>
  ...
  <style>
    ...
    *::after {
    ...
    .header-pill .material-icons-round {
      font-size: 17px;
      color: var(--primary)
    }
    ...
    .hero {
      ...
      display: flex;
      align-items: center;
      background: linear-gradient(135deg, #071d49 0%, #145bd8 47%, #05a9c7 100%);
      isolation: isolate
    }
    ...
    .order-product-preview img {
      ...
      object-fit: cover;
      border-radius: 15px;
      box-shadow: 0 10px 20px rgba(31, 50, 90, .12)
    }

    .order-product-preview h4 {
      font-size: 15px;
      font-weight: 900;
      margin-bottom: 4px
    }
